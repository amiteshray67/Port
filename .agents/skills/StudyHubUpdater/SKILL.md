...
description: >-
  ...
  PDF-to-JPG conversion, PAGE_IMAGES + INDEX updates, FTP deployment, and git
  commit/push. Trigger on "add this note to studyhub", "update the CFA hub",
  "add this explainer", "push new content to cfa-l2".
---
...
# Insert before closing brace of PAGE_IMAGES
content = re.sub(
...
# After reviewing output, assign pages per INDEX entry:
page_assignments = {
    "ec1": [1],
    "ec2": [5],
    "ec3": [2, 4],
...
-- Pull, commit, push
do shell script "cd '/Users/amiteshray/Documents/Claude/Projects/CFA PREP/_port_repo' && git add studyhub/ && git pull --rebase origin main 2>&1 && git commit -m 'feat(studyhub): add <Subject> notes + scanned pages' 2>&1 && git push origin main 2>&1"
```
...
### Append to seed_explainers.js
```javascript
{
  "id": "unique_id",
  "title": "Explainer Title",
  "topic": "equity",   // eco, fi, derivatives, corp, alternatives, quant, pm, ethics
  "type": "html",
...
After adding:
1. `node --check _port_repo/studyhub/assets/seed_explainers.js` — fix any errors
2. Bump cache-bust version in `cfa-l2.html`: `seed_explainers.js?v=X` → `?v=X+1`
...
> Cards for [Subject] now show scanned note pages. Git repo synced.
